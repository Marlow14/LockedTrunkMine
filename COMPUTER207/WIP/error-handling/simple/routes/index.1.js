...
// can also require the json file, insteading of fs
// http://localhost:3000/carjson?filename=db.json
router.get("/carjson", function (req, res, next) {
  let fileName = req.query.filename;
...
// or using lodash.where
http://localhost:3000/carjsonbycountry?filename=db.json&country=Italy
router.get("/carjsonbycountry", function (req, res, next) {
  let fileName = req.query.filename;
  ...
  fs.readFile(fileName, function (err, data) {
    if (err) {
      ...
      let cars = JSON.parse(data).Makes;
      let filered = cars.filter((e)=>{return e.make_country == country});
      res.send(`These cars are made in ${country}: ${JSON.stringify(filered)}`);     
    }
  });
});
...
// http://localhost:3000/unique?filename=db.json
router.get("/unique", function (req, res, next) {
  ...
  Promise.try(() => {
    ...
    cars.forEach(function(car) {
      console.log(car.make_country + " is being processed");
      let index = countries.indexOf(car.make_country);
      if(index == -1) {
        console.log(car.make_country + " added");
        countries.push(car.make_country);
        counters.push(1);
...
router.get('/cars/country', function (req, res) {
