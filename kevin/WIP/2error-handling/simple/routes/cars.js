...
router.get('/:id', function (req, res, next) {
  let contents = fs.readFileSync("./public/db.json");
  let jsonContent = JSON.parse(contents);
 // let id = JSON.parse(contents, function (key,value)
 // { if (key == "id") return value}
 // );

  let make = jsonContent.Makes;
 ...
 // let country = makes.make_country;
 // let id = makes.id;

 // console.log(`id is ${id}`);
 // console.log(`country is ${country}`);
    res.send(`car id is ${req.params.id}  `);
